...
export async function buildReportPdf(
  ...
  w.para(`Assessed on ${new Date(report.generatedAt).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}  ·  ${school.studentPopulation} students  ·  ${school.staffPopulation} staff`, {
    size: 9.5,
    color: GRAY,
    gapAfter: 18,
  });
  ...
  /* ── 30/60/90 plan ─────────────────────────────────────── */
  w.heading("The 90-Day Turnaround Plan");
  ...
  w.y -= 10;
  w.para(
    "This report was generated by KAEC's AI diagnostics from the school's own School Health Check responses. For implementation support, training or consultation, contact the KAEC team through the report page.",
    { size: 8, color: GRAY },
  );
