    ...
    return (
      <main className="grid min-h-screen place-items-center bg-slate-950 px-5 text-white">
        <div className="max-w-lg rounded-[30px] border border-white/10 bg-white/5 p-8 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-200">Use the same verified account connected to the school&apos;s KSHC report.</p>
    ...
    <main className="min-h-screen bg-slate-950 px-5 py-16 text-white">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-8 rounded-[34px] border border-white/10 bg-white/5 p-7 sm:p-10">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-200">{view.body}</p>
          <div className="mt-6 rounded-2xl border border-white/15 bg-black/20 p-5 text-sm text-slate-200">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            {partnership.partnerStatus === "active" && (
              <Link
                href={`/khpos/${partnership.organisationId}`}
                className="inline-flex items-center gap-2 rounded-full border border-mint-200 bg-mint-300 px-6 py-3 text-sm font-black text-slate-950 shadow-[0_10px_30px_rgb(16_185_129/0.24)] transition hover:bg-mint-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-mint-300"
              >
                Open KHP-OS <ArrowRight className="size-4" />
              </Link>
            )}
            <Link
              href="/assessment"
              className="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/5 px-6 py-3 text-sm font-black text-white transition hover:bg-white/10"
            >
              Run another KSHC
            </Link>
          </div>
