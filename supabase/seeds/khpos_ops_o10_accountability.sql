...
begin
  ...
  insert into public.khpos_ops_policy_versions(
    ...
    '["A minor, isolated accountability deviation may remain in PEO-012 where formal discipline would be disproportionate.","Recognition may be withdrawn only where its factual basis was materially wrong or the record was created in error; withdrawal does not rewrite the original audit history.","Outcome acknowledgement confirms receipt and never means forced agreement.","Where the highest internal authority is the subject, the case remains routed to independent external governance rather than being internally self-decided."]'::jsonb,
